...
## Hypotheses
(1) We anticipate that prevalence rates of disability or disorder will not change for diagnostically rigid categories (e.g., Autism Spectrum Disorder) and may plateau or decrease for diagnostically malleable categories (e.g., mild behavioural need).
(2) Allied professional resource support in the BC public education system points to challenges in rural settings. Therefore, we anticipate that the longitudinal provincial trend will largely be driven by urban school districts and rural districts may have highly variable outcomes post-2016 supreme court ruling. 
...
```
As observed in the figure above, low-incidence categories of need (e.g., deafblindness, visual impairment) remain constant over time. Both categorizations of intellectual disability saw a slight linear decline over time, while intensive behavioural need and learning disability categories had a slight linear increase over time. Chronic health impairment appeared to have a linear increase; though, no other category increased as rapidly as Autism throughout the more than two decades observed. 
...
# library(growthrates)
# 
# growthrate1 <- BCdatastripped_spedcat %>% 
#   filter(SPECIAL_NEEDS_CODE == "Autism")
# 
# all_easylinear(BCdatastripped_spedcat, grouping = "SPECIAL_NEEDS_CODE", time = "SCHOOL_YEAR", y= "NUMBER_OF_STUDENTS")
